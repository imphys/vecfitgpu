...
//
void get_zernikefunctions(double **orders, int size_orders, double **x, double **y, int XYpupil_size)
{
...
int zerpol[Nx][Ny][azormax + 4][azormax + 2];
double rhosq[XYpupil_size][XYpupil_size], rho[XYpupil_size][XYpupil_size], phi[XYpupil_size][XYpupil_size], allzernikes[Nx][Ny][Nzer];
...
for(int i = 0; i< Nx; i++)
{
  for(int j = 0; j < Ny; j++)
  {
    ...
    for(int k = 0; k < (radormax + 1); k++)
    {
      for(int l = 0; l < azormax + 1; l++)
      {
        if(k == 0 && l == 0)
        {
          zerpol[i][j][k][l] = 1;
...
for(int i = 0; i< Nx; i++)
{
  for(int j = 0; j < Ny; j++)
  {
    ...
    for(int jm = 0; jm < (azormax + 2); jm++)
    {
      ...
      ////
      ////NO SQUEEZE USED
      zerpol[i][j][jm+2][jm] = ((m + 2) * rhosq[i][j] - m - 1) * zerpol[i][j][jm][jm];
      for(int p = 1; p < radormax - m + 2; p++)
      {
        int n = m+2*p;
        int jn = n + 1;

        zerpol[i][j][jn][jm] = (2*(n-1)*(n*(n-2)*(2*rhosq[i][j]-1)-(m * m))*zerpol[i][j][jn-2][jm] - n*(n+m-2)*(n-m-2)*zerpol[i][j][jn-4][jm])/((n-2)*(n+m)*(n-m));
      }
